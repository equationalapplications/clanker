...
C4Context
  ...
  System_Ext(cloudagent, "Cloud Agent", "Stateless ADK agent on Cloud Run. Escalated chat for cloud-synced characters via WebSocket /agent/stream with HTTP /agent/run fallback")
  ...
  Rel(clanker, cloudagent, "Escalated chat for cloud-synced characters", "WebSocket /agent/stream (HTTP /agent/run fallback) + Bearer token")
