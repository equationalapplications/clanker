...
C4Context
  ...
  System_Ext(firebase, "Firebase", "Auth, Firestore session/task coordination bus, Cloud Functions (generateReply BYOI proxy, bootstrap, wiki LLM/sync, media callables, payment webhooks), Storage (character avatar images, GCS, uid-scoped), FCM sender for extension wake")
  ...
  Rel(clanker, firebase, "Auth, callable functions, App Check, Firestore reads (sessions/tasks/auth approvals), avatar image upload/download to Storage (save_to_cloud characters)")
