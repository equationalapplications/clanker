...
2. **No `initializeAnalytics()` / KV persistence step.** `crashlyticsService.ts` has an `initializeCrashlytics()` that reads a `'setting:analytics'` KV key at startup — but nothing in the codebase ever writes that key, so it's dead weight carried over from an earlier design. The approved spec's "Public interface" section lists exactly four functions (`logScreenView`, `logEvent`, `setAnalyticsEnabled`, `setUserId`) with no init function, so `analyticsService` does **not** replicate that pattern. Collection state is set only when `CookieConsentContext` calls `setAnalyticsEnabled` on consent change, exactly as specified.
