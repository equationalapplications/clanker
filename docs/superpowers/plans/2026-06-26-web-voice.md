...
**Architecture:** A single hook file (`useLiveAudioIO.web.ts`) implements the same `UseLiveAudioIOReturn` interface as the native version. Input: `getUserMedia` → `AudioContext({ sampleRate: 16000 })` → `AudioWorkletNode` (Float32→Int16 PCM, 20ms chunks sized from actual context rate, Blob URL loaded) → base64 → `onAudioChunk` listeners. Output: base64 PCM (24kHz from Gemini) → `AudioBufferSourceNode` gapless scheduling queue. No other runtime/product code changes — tests and docs are added/updated in this phase.
