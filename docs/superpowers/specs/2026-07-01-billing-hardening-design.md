...
**Fix:** compare `charge.amount_refunded` against the last processed refund total for that charge and deduct only the **newly refunded portion** on each `charge.refunded` event. Compute `targetTotalDeduction = floor(totalCredits * amount_refunded / amount)` and subtract the previously applied target so sequential partial refunds do not over-deduct. A full refund (`amount_refunded === amount`) still claws back the entire grant. Guard against division producing `NaN`/`Infinity` if `charge.amount` is ever `0`.
