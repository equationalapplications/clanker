...
A separate, related defect was discovered in native Google sign-in: [src/auth/googleSignin.ts](../../src/auth/googleSignin.ts) compares `error.code === 'statusCodes.SIGN_IN_CANCELLED'` (a literal string) instead of `statusCodes.SIGN_IN_CANCELLED` (the imported constant). The cancellation branch is dead code today, so cancellations fall through to a generic alert instead of being silenced — which conflicts with the new `cancelled: true` contract introduced on the `fix/google-fedcm-cancel-handling` branch.
...
- Fix the native Google `error.code` comparison so cancellations are correctly flagged with `cancelled: true`.
- Stop logging the full Google ID token response in `console.log`.

## Non-Goals

- One Tap on Android (Credential Manager / `GoogleOneTapSignIn`). Tracked as **Phase 2**, deferred to a separate spec because it requires a native build (BREAKING CHANGE per [AGENTS.md](../../../AGENTS.md)).
...
│     - Fix error.code to use statusCodes constants           │
│     - Replace full-response console.log with redacted log   │
└─────────────────────────────────────────────────────────────┘
```

### Phase 2 (separate spec, native build required)

- Adopt `GoogleOneTapSignIn` (Credential Manager API) on Android in `src/auth/googleSignin.ts`. iOS keeps the existing `GoogleSignin.signIn()` path.
...
### `src/auth/googleSignin.ts` (native, modified)

- `import { GoogleSignin, statusCodes } from '@react-native-google-signin/google-signin'`.
- Replace string-literal comparisons with constant comparisons:
  - `error.code === statusCodes.SIGN_IN_CANCELLED` → returns `{ success: false, cancelled: true, error: 'Sign-in was cancelled' }`.
  - `error.code === statusCodes.IN_PROGRESS` → returns `{ success: false, error: 'Sign in is already in progress' }`.
  - `error.code === statusCodes.PLAY_SERVICES_NOT_AVAILABLE` → returns `{ success: false, error: 'Play services not available or outdated' }`.
- Replace `console.log('🔍 Google Sign-In response:', JSON.stringify(response, null, 2))` with `console.log('🔍 Google Sign-In response received (idToken redacted)')`.
...
Same as iOS Google. Phase 2 will layer `GoogleOneTapSignIn` on top of this same actor path.
...
- `src/auth/__tests__/googleSignin.test.ts` (NEW)
  - Mocks `@react-native-google-signin/google-signin` exposing `statusCodes.SIGN_IN_CANCELLED`, `IN_PROGRESS`, `PLAY_SERVICES_NOT_AVAILABLE` matching the SDK's actual constant values.
  - Cancellation rejection → `{ success: false, cancelled: true, error: 'Sign-in was cancelled' }`.
  - In-progress rejection → `{ success: false, error: 'Sign in is already in progress' }` and `cancelled` is `undefined`.
  - Play-services rejection → `{ success: false, error: 'Play services not available or outdated' }`.
  - Successful sign-in: `signInWithCredential` called with the Google credential; `syncDisplayNameFromCredential` called with the firebase user and the derived display name; result is `{ success: true }`.
  - Asserts no `console.log` call in the success path contains the raw response payload (regex against `console.log` calls confirms the diagnostic-token-leak fix).
...
- **Risk:** Phase 2 (Android One Tap) lands on a future native build and could regress sign-in.
  - **Mitigation:** Phase 1 leaves the native flow intact. Phase 2 will be its own spec with classic `GoogleSignin.signIn()` retained as a documented fallback.
...
Tracked here for visibility; out of scope for this branch:

- Replace Android `GoogleSignin.signIn()` with `GoogleOneTapSignIn` (Credential Manager). Falls back to classic `GoogleSignin.signIn()` when One Tap is unavailable.
- Requires a native rebuild; commit must be marked `BREAKING CHANGE` per [AGENTS.md](../../../AGENTS.md).
- Will get its own spec under `docs/superpowers/specs/`.

## Self-review notes:

- All sections from the brainstorm are present (Problem, Goals, Non-Goals, Architecture w/ Phase 1 diagram, Components, Data Flow, Error Handling, Testing, Configuration, Risk, Deferred Phase 2).
- No placeholder text or TODOs.
- One internal inconsistency to flag: I described <GoogleSignInButton.web /> attaching a ref to react-native-web's <View> (which renders to a div). This is correct but slightly hand-wavy — the implementation may end up using a raw <div> via Platform-specific code or dangerouslySetInnerHTML-style escape hatch. Acceptable to leave as a design-level note; the executing plan can choose the exact mechanism.
- signInProvider actor is intentionally left symmetric across platforms (web Google branch becomes unreachable rather than removed) — called out explicitly to avoid surprising the implementer.
Configuration section confirms no Firebase/Google Cloud changes.