...
4. **Hash verification** — recompute SHA-256 of (post-truncation) content; if mismatch with client-supplied `contentHash` → `HttpsError('invalid-argument', 'Content hash mismatch')`. Returned `contentHash` is the server-recomputed value (authoritative).
