...
**Status:** Implemented — Phases 0–4 in PRs #603, #604, #605, #606, #607. Phases 0–3 merged to `staging`; Phase 4 PR #607 open and awaiting user merge. Promotion PR (`staging` → `main`) is the user's gate after Phase 4 merges and Phase 1 production deploy verifies. See [Implementation log](#implementation-log-2026-08-12).
...
| ----------------------------- | ------------------------------------------------------------------ | -------------------------------------- | ---------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| 0 — Internal package lockstep | [#603](https://github.com/equationalapplications/clanker/pull/603) | `chore/deps-phase-0-internal-packages` | Merged           | All four `@equationalapplications/*` packages aligned on `^5.2.1` (chose 5.x over 4.23.1 after verifying 5.x surface is purely additive). `core-llm-tools` moved from exact pin to caret range. Zero source/test changes — 5.x surface is purely additive.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                   |
| 1 — Backend majors            | [#604](https://github.com/equationalapplications/clanker/pull/604) | `chore/deps-phase-1-backend-majors`    | Merged           | ADK 1.6 + genai 2.x (one step), firebase-admin 14, express 5 + @types/express 5, @types/node 24, `functions/` ESLint 10 + `@eslint/js` 10, @types/supertest 7. Required creating `firebaseAdmin.ts` helpers in both backends to preserve the existing `withAdminAuthStub` mocking pattern in ESM. Two declared-but-unused devDeps (`eslint-plugin-import`, `firebase-functions-test`) removed as dead weight blocking their respective upgrades. The `@google/adk: { js-yaml: 4.3.0 }` scoped override was removed in Phase 1 since ADK 1.6.0's natural `js-yaml@^4.1.1` resolution already meets the advisory threshold. All 7 WebSocket + 5 CORS HTTP regression tests verified individually. Follow-up fixes: prettier `--check` on 4 modified files (commit `a0647121`); CodeRabbit auth-cache `??=` initialization bug fix in `withAdminAuthPartialStub` via new `__getAuthRawForTest()` accessor (commit `381fdd91`). |
| 2 — Root build/test tooling   | [#605](https://github.com/equationalapplications/clanker/pull/605) | `chore/deps-phase-2-root-tooling`      | Merged (partial) | Shipped: avatarPicker fake-timer fix (removes `jest.setTimeout(30_000)` workaround), commitlint 21, @types/node 24 (deliberate downward from 25), better-sqlite3 13, semantic-release 25.0.9 (rejected audit's 24.2.9 downgrade). **Deferred** due to ecosystem blockers: @babel/core 8 (`@babel/plugin-syntax-dynamic-import` and ~10 siblings still on `^7.x`; `babel-preset-expo@58.0.0-canary-20260806` blocks), Jest 30 + RNTL 14 (`jest-expo@57.0.4` hard-pins `jest@29.x` deps; causes 156/156 suite failures from `jest-mock` version conflict), root ESLint 10 (`eslint-config-expo@57.0.1` bundles `eslint-plugin-react@7.37.5` which peers `eslint <=9.7`). Recommended follow-up: Phase 2.5 once `eslint-config-expo` drops `eslint-plugin-react@^7` for `^8`, since that release likely unblocks Babel 8, Jest 30, and ESLint 10 together.                                                           |
| 3 — Root runtime and native   | [#606](https://github.com/equationalapplications/clanker/pull/606) | `chore/deps-phase-3-runtime-native`    | Merged           | All 7 `@react-native-firebase/*` → 26.2.0, `react-native-error-boundary` 3, `zod-validation-error` 5, `expo-age-range` 57.0.3 with regenerated patch, `expo-speech-recognition` removed with microphone permission transplant to `app.config.ts`. Verified by `npx expo prebuild --clean` + grepping the generated `Info.plist` and `AndroidManifest.xml` — the transplant is invisible to `npm test`/`typecheck`/OTA and would crash the Talk tab if shipped wrong. **Deferred**: `react-native-gesture-handler` 2→3 and `react-native-webview` 13→14 because `npx expo install --fix` reverts them to SDK 57-pinned versions (need SDK 58). Native builds (iOS/Android) and on-device Talk tab verification are the user's gate.                                                                                                                                                                           |
| 4 — Overrides sweep           | [#607](https://github.com/equationalapplications/clanker/pull/607) | `chore/deps-phase-4-overrides`         | Open             | Three new rationale docs (`docs/dependency-overrides.md`, `docs/cloud-agent/dependency-overrides.md`, `docs/functions/dependency-overrides.md`). Audit summary: removed dead overrides (`@babel/core`, `postcss` at root; `ws`, `protobufjs`, `@babel/core` in `functions/`); kept `uuid: ^11.1.1` in `functions/`; added scoped `sqlite3: { tar: "7.5.22" }` in both backends (clears the critical `tar` advisory in the `drizzle-orm → sqlite3 → node-gyp → tar` chain) and `uuid: ^11.1.1` in `cloud-agent`. Final audit: root 34 (unchanged — all build-time, accepted), cloud-agent 29→18, functions 24→22. **0 critical** (was 2); 4 high residual unfixable without an `@google/adk` major that adopts newer transitive peers.                                                                                                                                                                        |

### Promotion — staging → main

Phases 0–3 are merged to `staging`; Phase 4 (#607) is open and awaiting user merge. After Phase 4 merges, a single promotion PR (`staging` → `main`) opens the OTA `runtimeVersion` fence once (30.0.0 → 31.0.0) — see [Rollout](#rollout). Backend majors (Phase 1) deployed straight to production when Phase 1 PR merged, and have been live for the duration of Phases 2–4 — that production soak is by design (Phase 1 is ordered early in the spec for exactly this reason).
