...
| **CI (`staging-test.yml`)**    | `prettier --write` + `eslint --fix` before tests (writers, cannot fail); no typecheck anywhere; `functions/` never tested; Node 22 | `format:check` + `lint:check` (gates); typecheck in all three workspaces; `functions/` install+typecheck+test added; Node 24; `patch-package --error-on-fail`                 |
