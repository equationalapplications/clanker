...
| `uuid` (transitive via `npm`)                                                                                                                                                 | moderate        | Build host only. `<11.1.1`, `>=12.0.0 <12.0.1`, `>=13.0.0 <13.0.1` vulnerable (advisory fully resolved in `14.0.0`); natural resolution `7.0.3` (a long-EOL major, inside the `<11.1.1` range). Adding a root override here would force every transitive `uuid` in the build tooling to a single major. We accept this residue; CI is short-lived and not exposed to the internet.                                                                                                                        |
