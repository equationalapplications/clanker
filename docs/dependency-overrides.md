...
deleted in Phase 4 — see [Removed overrides](#removed-overrides) below.

## Removed overrides (Phase 4 audit)

| Override               | Removed because                                                                                                                                                                                                                                                                                                                                                                                            |
| ---------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `@babel/core: ^7.29.7` | **Dead override.** Natural resolution is `7.29.7` (matches `devDependencies` entry on line 112). Removing the override does not change the resolved version. No advisory at this version. The override was masking intent rather than fixing anything. Removal condition was: any of (a) `@babel/core` introduced at a different version range, (b) an advisory appears at `7.29.7`, or (c) Babel 8 lands. |
| `postcss: 8.5.24`      | **Dead override.** Natural resolution with the override is `8.5.24`; without the override, `8.5.26` resolves — both versions carry **no `npm audit` advisory**, and the audit count is identical (34 vulnerabilities) in both states. The override was an artifact of pinning against `expo@57 → @expo/metro-config@57.0.8`'s older postcss dependency, which has since caught up.                         |
...
subtree, `expo-cli` / `@expo/*` tooling) per the spec's rule 7 ("Root alerts
...
| `tar` (in `npm`/`pacote`/`libnpmdiff`)                                                                                                                                        | moderate        | Build host only (`npm pack`/`npm publish`). The vulnerable range `<=7.5.20`; natural resolution `7.5.19`. Per rule 7, build-time risk is weighed against forcing a version Metro was not tested with — leaving it on the build host. **Not** added as an override.       |
