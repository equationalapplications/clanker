...
| 28      | `0028_scheduled_wakeups_delivery_mode.sql`         | Promotes the delivery mode out of the free-text `outcome` into `scheduled_wakeups.delivery_mode` (effective, post-clamp) and `chosen_delivery_mode` (model's pre-clamp choice) — two columns, since the chosen distribution is what the rollout gate tunes caps against. Backfills both from `outcome` under `IS NULL` guards (re-runnable) and adds `scheduled_wakeups_character_delivery_idx`. Scheduler Phase 2 (PR #702). Requires `0026`. Applied to prod 2026-09-09 with pre-backup |
| 29      | `0029_messages_read_at.sql`                        | `messages.read_at` (nullable `timestamptz`) for server-authoritative read state — `proactiveWakeupGuardrails` gates pushes on `unreadProactiveCount === 0`, which the sweeper must evaluate with no device in the loop. Partial index `messages_character_unread_idx WHERE read_at IS NULL`. Scheduler Phase 2 (PR #702). Applied to prod 2026-09-09 with pre-backup (same run as `0028`)                                                                                                 |

| 30 | `0030_users_proactive_push_ready.sql` | `users.proactive_push_ready` (`boolean NOT NULL DEFAULT false`) — per-user push-readiness capability for the proactive lifecycle-sync (Decision 1). `registerExpoPushToken` writes it explicitly in both directions (true when `capabilities.proactivePush === true`, false otherwise); the gate fires pushes only for capable clients instead of every client that ever registered a token. Additive, idempotent, no backfill required. Scheduler Phase 2 (PR #710). Applied to prod 2026-09-10 with pre-backup — same incident class as #1 above (migration merged but not run before deployed functions started reading the column; `exchangeToken`'s `SELECT ... proactive_push_ready` failed with `42703` and the web auth bootstrap looped on bootstrap→signedOut→USER_FOUND until the column existed) |
