...
} from './schedulerTriggerHandler.js'
...
function buildApp(
  ...
  const mockFs = {
    getActiveDevice:
      overrides.getActiveDevice ??
      (async () => ({ deviceId: 'd1', fcmToken: 'fcm-tok', deviceName: 'Mac' })),
    ...
    reserveSchedulerRun:
      overrides.reserveSchedulerRun ??
      (async (uid: string, runKey: string, ids: { sessionId: string; taskId: string }) => {
    ...
    getSchedulerRun:
      overrides.getSchedulerRun ??
      (async (uid: string, runKey: string) => {
        return schedulerRuns.get(`${uid}:${runKey}`) ?? null
      }),
    watchTask:
      overrides.watchTask ??
      ((_u: string, _s: string, _t: string, cb: (t: TaskDoc) => void) => {
        setTimeout(
          () =>
            cb({
              status: 'complete',
              result: { data: { price: '$340' }, activeUrl: 'https://x.com' },
              error: null,
  ...
  const mockFcm = {
    wakeExtension: async () => {},
    sendProactive:
      overrides.sendProactive ??
      (async (token: string, sid: string, tid: string, body: string) => {
  ...
  const mockCredit = {
    spendCredit:
      overrides.spendCredit ??
      (async () => {
        creditCalls.spend++
        return [{ transactionId: 'tx1', amount: 1 }]
      }),
    refundCredit:
...
test('returns 401 with no Authorization header', async () => {
  const { app } = buildApp()
  const res = await request(app).post('/agent/browser/scheduler-trigger').send(schedulerBody)
...
test('returns 422 for blocked host', async () => {
  ...
  const res = await request(app)
    ...
    .send({
      ...schedulerBody,
      action: { type: 'open_tab', url: 'chrome://settings' },
      actionSummary: 'Open',
    })
  assert.equal(res.status, 422)
...
test('returns 402 when user has insufficient credits', async () => {
  ...
  const res = await request(app)
    ...
    .send({
      ...schedulerBody,
      runKey: 'job-insufficient-1',
      action: { type: 'extract', selector: '.price', label: 'price' },
      actionSummary: 'Extract price',
      notificationBody: 'Price check done.',
    })
        ...
        () =>
          cb({
            ...
            error: {
              ...
              message: 'not found',
              failedAction: { type: 'extract', selector: '.p' } as never,
            },
            intent: { action: { type: 'extract', selector: '.p' } },
          } as unknown as TaskDoc),
        5,
