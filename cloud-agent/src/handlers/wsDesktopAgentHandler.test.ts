...
function fakeSession(over: Record<string, unknown> = {}) {
  const calls: Record<string, unknown[][]> = {
    markDesktopDeviceOnline: [],
    markDesktopDeviceOffline: [],
    touchDesktopDeviceLastSeen: [],
    markDesktopTaskExecuting: [],
    writeDesktopTaskResult: [],
  ...
  const fs = {
    calls,
    emitPending(tasks: Array<Record<string, unknown>>) {
      pendingCb?.(tasks)
    },
    emitDevice(doc: { exists: boolean; isPaused: boolean }) {
      deviceCb?.(doc)
    },
    markDesktopDeviceOnline: async (...a: unknown[]) => {
      calls.markDesktopDeviceOnline.push(a)
    },
    markDesktopDeviceOffline: async (...a: unknown[]) => {
      calls.markDesktopDeviceOffline.push(a)
    },
    touchDesktopDeviceLastSeen: async (...a: unknown[]) => {
      calls.touchDesktopDeviceLastSeen.push(a)
    },
    markDesktopTaskExecuting: async (...a: unknown[]) => {
      calls.markDesktopTaskExecuting.push(a)
    },
    writeDesktopTaskResult: async (...a: unknown[]) => {
      calls.writeDesktopTaskResult.push(a)
    },
    failDesktopTaskIfUnresolved: async (...a: unknown[]) => {
      calls.failDesktopTaskIfUnresolved.push(a)
      return true
    },
    watchPendingDesktopTasks: (
      _u: string,
      _d: string,
      cb: (t: Array<Record<string, unknown>>) => void,
    ) => {
      pendingCb = cb
      return () => {
        pendingCb = null
...
function options(
  fs: ReturnType<typeof fakeSession>,
  ...
  return {
    ...
    resolvePairingToken: async (raw: string) =>
      raw === 'good-token' ? { uid: 'u1', deviceId: 'desk1' } : null,
...
function authFrame() {
  return JSON.stringify({ type: 'auth', pairingToken: 'good-token' })
}
async function tick() {
...
test('bad pairing token closes 4001', async () => {
  ...
  const fs = fakeSession()
  handleDesktopWsUpgrade(ws as never, {} as never, options(fs) as never)
  ws.emit('message', Buffer.from(JSON.stringify({ type: 'auth', pairingToken: 'wrong' })))
  await tick()
...
test('task_result writes complete result; task_error writes failure', async () => {
  ...
  await tick()
  fs.emitPending([
    { taskId: 't1', tool: 'wiki_search', params: {}, status: 'pending', deviceId: 'desk1' },
  ])
  await tick()
  ws.emit(
    'message',
    Buffer.from(JSON.stringify({ type: 'task_result', taskId: 't1', result: { hits: 2 } })),
  )
  await tick()
  assert.deepEqual(fs.calls.writeDesktopTaskResult[0], [
    'u1',
    't1',
    { status: 'complete', result: { hits: 2 } },
  ])

  fs.emitPending([
    { taskId: 't2', tool: 'wiki_search', params: {}, status: 'pending', deviceId: 'desk1' },
  ])
  await tick()
  ws.emit(
    'message',
    Buffer.from(
      JSON.stringify({ type: 'task_error', taskId: 't2', error: { code: 'E', message: 'boom' } }),
    ),
  )
  await tick()
  assert.deepEqual(fs.calls.writeDesktopTaskResult[1], [
    'u1',
    't2',
    { status: 'failed', error: { code: 'TOOL_ERROR', message: 'boom' } },
  ])
})
...
test('close marks device offline and fails in-flight dispatched tasks', async () => {
  ...
  assert.equal(
    (fs.calls.failDesktopTaskIfUnresolved[0][2] as { code: string }).code,
    'DESKTOP_DISCONNECTED',
  )
})

test('shortcut dispatchTask enters dispatched set: fails DESKTOP_DISCONNECTED on close', async () => {
  ...
  ws.emit('message', Buffer.from(authFrame()))
  await tick()
  const accepted = bridge.get('u1', 'desk1')!.dispatchTask('t-local', 'wiki_search', { query: 'x' })
  ...
  await tick()
  assert.equal(fs.calls.failDesktopTaskIfUnresolved.length, 1)
  assert.equal(fs.calls.failDesktopTaskIfUnresolved[0][1], 't-local')
  assert.equal(
    (fs.calls.failDesktopTaskIfUnresolved[0][2] as { code: string }).code,
    'DESKTOP_DISCONNECTED',
  )
...
test('shortcut dispatchTask dedupes against the pending-queue listener (no double send)', async () => {
  const bridge = createDesktopBridge()
  const ws = new FakeWs()
  const fs = fakeSession()
  handleDesktopWsUpgrade(ws as never, {} as never, options(fs, bridge) as never)
  ws.emit('message', Buffer.from(authFrame()))
  await tick()
  bridge.get('u1', 'desk1')!.dispatchTask('t1', 'wiki_search', { query: 'x' })
  fs.emitPending([
    {
      taskId: 't1',
      tool: 'wiki_search',
  ...
  await tick()
  assert.equal(ws.frames().filter((f) => f.type === 'task').length, 1)
  assert.equal(fs.calls.markDesktopTaskExecuting.length, 1)
})
...
test('replacement race: stale close after new registration must NOT mark device offline', async () => {
  ...
  ws1.emit('message', Buffer.from(authFrame()))
  await tick()

  const ws2 = new FakeWs()
  handleDesktopWsUpgrade(ws2 as never, {} as never, options(fs, bridge) as never)
  ws2.emit('message', Buffer.from(authFrame()))
  await tick()
