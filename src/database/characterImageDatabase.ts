 ...
 * not confirmed.
 *
 * Scoped by `(character_id, user_id)` even though `message_id` is the lookup
 * key: `message_id` is not a foreign key and is generated by the client, so a
 * collision (in practice a UUID clash; theoretically a synced row carrying a
 * duplicate id) would otherwise surface another character/user's photo. The
 * partial index on `message_id` is unchanged — the planner still picks it up
 * for the `message_id = ?` predicate.
 */
export async function findCharacterImageByMessageId(
  ...
  return db.getFirstAsync<CharacterImageRow>(
    `SELECT * FROM character_images
     WHERE message_id = ? AND character_id = ? AND user_id = ?
       AND deleted_at IS NULL AND sync_state != 'reserved'
     ORDER BY created_at DESC
     LIMIT 1`,
    [messageId, characterId, userId],
  )
}
