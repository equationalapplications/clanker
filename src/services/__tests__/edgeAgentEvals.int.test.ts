...
describe('Edge Agent LLM Routing Evals', () => {
  it('Test A: asking about a past fact yields a search_memory tool call', async () => {
  ...
  it('Test B: asking to write a long essay yields an escalate_to_cloud_agent tool call or a cloud-only escalation response', async () => {
    const result = await runEdgeEval(
    ...
    if (calls.length > 0) {
      ...
      expect(result.text?.toLowerCase()).toMatch(
        /cloud|escalate|remote server|server-side|more powerful tool|connect(?:s|ed)? you with|transfer(?:red)? you|tool that specializes/,
      )
    }
  }, 30000)

  it('Test C: casual chatting yields a text response with no tool calls', async () => {
