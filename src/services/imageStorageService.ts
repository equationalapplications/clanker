...
export const generateAndStoreCharacterImage = async ({
    ...
    try {
        ...
        if (fileSizeKB > maxFileSizeKB) {
            console.warn(`⚠️ Image size (${fileSizeKB.toFixed(2)}KB) exceeds limit (${maxFileSizeKB}KB), but proceeding`)
        }
 ...
 */
export const getSignedImageUrl = async (storagePath: string, expiresIn: number = 3600): Promise<string> => {
    try {
    ...
    return new Promise((resolve, reject) => {
        img.onload = async () => {
            ...
            while (attempts < maxAttempts) {
                ...
                console.log(`🔄 Compression attempt ${attempts + 1}: ${fileSizeKB.toFixed(2)}KB at ${quality}% quality`)
 ...
 */
export const listCharacterImages = async (userId: string, characterId?: string): Promise<string[]> => {
    try {
