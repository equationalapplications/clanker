...
describe('IngestProgressBar', () => {
  it('renders null when progress is 0', () => {
    ...
    act(() => {
      tree = create(<IngestProgressBar progress={0} />)
    })
  ...
  it('renders null when progress is negative', () => {
    let tree!: ReturnType<typeof create>
    act(() => {
      tree = create(<IngestProgressBar progress={-0.1} />)
    })
    expect(tree.toJSON()).toBeNull()
  })
  ...
  it('renders a bar when progress > 0', () => {
    let tree!: ReturnType<typeof create>
    act(() => {
      tree = create(<IngestProgressBar progress={0.5} />)
    })
  ...
  it('clamps width to 100% at progress=1.5', () => {
    let tree!: ReturnType<typeof create>
    act(() => {
      tree = create(<IngestProgressBar progress={1.5} />)
    })
