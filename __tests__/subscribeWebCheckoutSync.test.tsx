...
jest.mock('react-native-paper', () => {
  ...
  const Button = ({ children, onPress, ...props }: any) => (
    <Pressable
      testID={props.testID ?? (typeof children === 'string' ? children : undefined)}
      onPress={onPress}
      {...props}
    >
...
jest.mock('~/components/CreditsDisplay', () => (props: any) => {
  ...
  return React.createElement(
    'Text',
    { testID: 'credits-display-locks' },
    JSON.stringify(props.webCheckoutLocks),
  )
})
...
describe('Subscribe web checkout sync', () => {
  ...
  beforeEach(() => {
    ...
    mockReadCheckoutAttempts.mockImplementation((uid: string) => ({
      ...(attemptsByUid[uid] ?? {}),
    }))
  ...
  it('expires stale pending attempts on focus recovery and surfaces the timeout message', async () => {
    ...
    expect(mockExpireStalePendingAttempts).toHaveBeenCalledWith(
      'user-1',
      expect.any(Number),
      'tab-recovery',
    )
  ...
  it('on web, only subscribe screen shows timeout snackbar, not CreditsDisplay', async () => {
    ...
    expect(
      mockCreditsDisplayRender.mock.calls[mockCreditsDisplayRender.mock.calls.length - 1][0]
        .expiredMessage,
