...
$('signout').addEventListener('click', () => {
  void signOut(auth)
  void chrome.storage.local.remove('deviceId')
})
$('retry-register').addEventListener('click', () => {
...
$('grant').addEventListener('click', async () => {
  const { pendingHost, pendingOrigin } = await chrome.storage.local.get([
    'pendingHost',
    'pendingOrigin',
  ])
...
chrome.storage.onChanged.addListener((c) => {
  if (c.pendingHost) void syncGrantButton()
})
...
async function registerThisDevice(): Promise<void> {
  ...
  try {
    ...
    const { deviceId: existing, gcmToken: cached } = await chrome.storage.local.get([
      'deviceId',
      'gcmToken',
    ])
    ...
    if (!res.ok) {
      $('device').textContent = `Device: registration failed (${res.status})`
      return
    }
