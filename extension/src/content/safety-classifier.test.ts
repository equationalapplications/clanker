...
function el(html: string) {
  return new JSDOM(`<body>${html}</body>`).window.document.body.firstElementChild!
}
...
test('button-type input with destructive value → requires_auth', () => {
  assert.equal(
    classifyElement(el('<input type="button" value="Delete account" />')),
    'requires_auth',
  )
})
...
test('input with destructive aria-label → requires_auth', () => {
  assert.equal(
    classifyElement(el('<button aria-label="submit payment">Go</button>')),
    'requires_auth',
  )
})
