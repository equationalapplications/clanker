...
function injector(results: Record<string, { data: Record<string, string>; activeUrl: string }>) {
  ...
  return {
    seen,
    runInActiveTab: async (action: { type: string }) => {
      seen.push(action.type)
      return results[action.type]
    },
    openTab: async (_url: string) => {
      seen.push('open_tab')
    },
    focusTab: async (_host: string) => {
      seen.push('focus_tab')
    },
  }
}
  ...
  const res = await dispatchTask(
    {
      ...
      requiresAuth: false,
      actionSummary: 'x',
      action: { type: 'extract', selector: '.x' },
    },
    inj as never,
  )
...
test('dispatchTask halts with awaiting_auth when step returns awaitingAuth', async () => {
  ...
  assert.deepEqual((outcome as { partialData: Record<string, string> }).partialData, {
    price: '$10',
  })
})
