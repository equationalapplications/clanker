...
export type AwaitingAuthOutcome = {
...
export async function dispatchTask(intent: TaskIntent, inj: Injector): Promise<DispatchOutcome> {
  const steps: SingleAction[] =
    intent.action.type === 'sequence' ? intent.action.steps : [intent.action]
  const skipLayerTwoForFirst =
    intent.authApproved === true &&
    steps.length > 0 &&
  ...
  try {
    for (let i = 0; i < steps.length; i++) {
      ...
      if (step.type === 'open_tab') {
        await inj.openTab(step.url)
        activeUrl = step.url
        continue
      }
      if (step.type === 'focus_tab') {
        await inj.focusTab(step.host)
        continue
      }
