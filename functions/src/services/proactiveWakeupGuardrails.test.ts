...
test('permits notify exactly at the cooldown boundary', () => {
  ...
  const decision = decideWakeup(input({ lastUserMessageAt: boundary })) as { run: true; notifyAllowed: boolean }
...
test('forbids notify once the daily push ceiling is reached', () => {
  const decision = decideWakeup(input({ todaysPushCount: 2 })) as { run: true; notifyAllowed: boolean }
  assert.equal(decision.notifyAllowed, false)
})

test('permits notify when the user has never sent a message', () => {
  const decision = decideWakeup(input({ lastUserMessageAt: null })) as { run: true; notifyAllowed: boolean }
  assert.equal(decision.notifyAllowed, true)
})
