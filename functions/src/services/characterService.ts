...
export function buildCharacterUpdateValues(character: CharacterUpdateInput) {
  const normalizedVoice =
    character.voice == null ? undefined : character.voice === '' ? DEFAULT_VOICE : character.voice
  ...
  const withSaveToCloud = {
    ...updateValues,
    ...(character.saveToCloud !== undefined && { saveToCloud: character.saveToCloud }),
  }
...
export const createCharacterService = (deps: CharacterServiceDeps = { getDb }) => {
  const service = {
