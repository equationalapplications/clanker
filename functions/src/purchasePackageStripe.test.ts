    ...
    await assert.rejects(
      async () =>
        purchasePackageStripeHandler(
          ...
          { userRepository: { findUserByFirebaseUid: async () => null } } as never,
        ),
      (err: unknown) =>
        err instanceof HttpsError &&
        err.code === 'failed-precondition' &&
...
test('purchasePackageStripeHandler creates a customer when none exists', async (t) => {
  ...
  const listCustomersMock = t.mock.method(
    customersPrototype,
    'list',
    async () =>
      ({
  ...
  const createCustomerMock = t.mock.method(
    customersPrototype,
    'create',
    async () =>
      ({
...
test('purchasePackageStripeHandler fails when Stripe checkout session has no URL', async (t) => {
  stubHandlerDeps(t, 'recurring', 'cs_no_url', '')
