...
test('wikiSync: accepts valid dump for premium user', async () => {
  ...
  const validateEntityOwnership = async () => {
    /* ownership validated by test setup */
  }
...
test('wikiSync: last-write-wins semantics via upsertData injection', async () => {
  ...
  assert.equal(
    store.get('lww-fact')?.body,
    'newer body',
    'stale update must not overwrite newer version',
  )
})
...
test('wikiSync: propagates tombstones (deleted_at) for cross-device deletion', async () => {
  ...
  const result = await wikiSyncHandler(
    { auth, data: { dump: importDump } } as unknown as CallableRequest,
    {
      upsertData,
      validateEntityOwnership,
      fetchMergedDump,
...
test('wikiSync: rejects invalid confidence value', async () => {
  ...
  await rejectsFact(
    user,
    auth,
    buildFact(TEST_ENTITY_UUID, { confidence: 'unknown' }),
...
test('wikiSync: rejects invalid source_type value', async () => {
  ...
  await rejectsFact(
    user,
    auth,
    buildFact(TEST_ENTITY_UUID, { source_type: 'bad_type' }),
...
test('wikiSync: rejects float last_accessed_at', async () => {
  ...
  await rejectsFact(
    user,
    auth,
    buildFact(TEST_ENTITY_UUID, { last_accessed_at: 1.5 }),
    /last_accessed_at must be an integer/,
  )
})
...
test('wikiSync: rejects NaN last_accessed_at', async () => {
  ...
  await rejectsFact(
    user,
    auth,
    buildFact(TEST_ENTITY_UUID, { last_accessed_at: NaN }),
    /last_accessed_at must be an integer/,
  )
})
...
test('wikiSync: rejects Infinity last_accessed_at', async () => {
  ...
  await rejectsFact(
    user,
    auth,
    buildFact(TEST_ENTITY_UUID, { last_accessed_at: Infinity }),
    /last_accessed_at must be an integer/,
  )
})
...
test('wikiSync: rejects float access_count', async () => {
  ...
  await rejectsFact(
    user,
    auth,
    buildFact(TEST_ENTITY_UUID, { access_count: 1.5 }),
...
test('wikiSync: rejects negative access_count', async () => {
  ...
  await rejectsFact(
    user,
    auth,
    buildFact(TEST_ENTITY_UUID, { access_count: -1 }),
    /access_count must be a non-negative integer/,
  )
})
    ...
    entities: {
      [TEST_ENTITY_UUID]: {
        ...
        edges: [
          {
            id: 'e1',
            entity_id: TEST_ENTITY_UUID /* missing source_id, target_id, edge_type, created_at */,
          },
        ],
      },
    },
...
test('wikiSync: ontology bundle round-trips through upsertData/fetchMergedDump unchanged', async () => {
  ...
  assert.deepEqual(receivedOntology, {
    mode: 'emergent',
    manifest: { node_types: [], edge_types: [] },
  })
  assert.deepEqual(result.remoteDump.entities[TEST_ENTITY_UUID]?.ontology, {
    mode: 'emergent',
    manifest: { node_types: [], edge_types: [] },
  })
})
