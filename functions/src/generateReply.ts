...
import {
  getGenAIClient as getSharedGenAIClient,
  isRetryableEmptyResponseFinishReason,
} from './services/vertexText.js'
import {
...
function validateStructuredPayloadSize(contents: unknown[], systemInstruction: string): void {
  ...
  try {
    ...
    throw new HttpsError('invalid-argument', 'Structured contents must be JSON-serializable.')
  }
...
function validateStructuredContents(contents: unknown[]): void {
  for (const [index, item] of contents.entries()) {
    ...
    if (parts.length === 0) {
      throw new HttpsError('invalid-argument', `contents[${index}].parts must not be empty.`)
    }
...
function validateTools(tools: unknown[]): ToolDeclaration[] {
  return tools.map((tool, index) => {
    ...
    if (!ALLOWED_TOOL_NAMES.has(t.name)) {
      throw new HttpsError(
        'invalid-argument',
        `tools[${index}].name "${t.name}" is not a recognized tool.`,
      )
    }
...
function trimSystemInstruction(
      ...
      const shouldRetry =
        ...
        (candidates.length === 0 ||
          candidates.some((candidate) =>
            isRetryableEmptyResponseFinishReason(candidate.finishReason),
          ))
...
function parseInput(data: unknown): {
  ...
  if (!prompt && contents === undefined) {
    throw new HttpsError('invalid-argument', 'prompt or structured contents are required.')
  }
