...
test('memoryWriteHandler does NOT fall back to heuristic when LLM returns valid empty result', async () => {
  const deps = {
    ...buildDeps(),
    generateContent: async (_prompt: string) => JSON.stringify({ entries: [], tasks: [] }),
  }
          ...
          set() {
            return {
              where(condition: unknown) {
                capturedWhere = condition
                return {
                  returning: async () => [{ id: 'entry-1' }, { id: 'entry-2' }, { id: 'entry-3' }],
                }
              },
            }
          },
...
test('memoryHealHandler does not downgrade or delete user_document entries', async () => {
  ...
  const diff = result.diff as {
    ...
    entries: Array<{ id: string; deletedAt: null | string; confidence: string }>
  }
  ...
  if (updatedDocEntry) {
    assert.equal(
      updatedDocEntry.deletedAt,
      null,
      'user_document entry deletedAt should remain null',
    )
    assert.equal(
      updatedDocEntry.confidence,
      'inferred',
      'user_document entry confidence should not change',
    )
  }
})
...
test("memory generateContent: throws (not '') on empty after retry; budget 1024", async () => {
  ...
  assert.deepEqual((seenConfigs[0] as Record<string, unknown>)['thinkingConfig'], {
    thinkingBudget: 1024,
  })
