...
function parseForgetTargets(data: unknown): {
  ...
  if (data.sourceRef !== undefined) {
    ...
    const cleaned = data.sourceRef.replace(/[/\\]/g, '').split('\0').join('').trim().slice(0, 255)
  ...
  if (data.sourceHash !== undefined) {
    if (typeof data.sourceHash !== 'string' || !/^[0-9a-f]{64}$/i.test(data.sourceHash)) {
      throw new HttpsError(
        'invalid-argument',
        'sourceHash must be a 64-character hex SHA-256 string.',
      )
    }
  ...
  return raw
    ...
    .map((item): MemoryWriteEntry | null => {
      ...
      const confidence: MemoryWriteEntry['confidence'] =
        item['confidence'] === 'certain' || item['confidence'] === 'tentative'
          ? item['confidence']
          : 'inferred'
      const sourceType: MemoryWriteEntry['sourceType'] =
        item['sourceType'] === 'user_stated' ||
        item['sourceType'] === 'user_confirmed' ||
        item['sourceType'] === 'user_document'
          ? item['sourceType']
          : 'agent_inferred'
      ...
      const lastAccessedAt =
        typeof item['lastAccessedAt'] === 'number' ? item['lastAccessedAt'] : null
      const accessCount =
        typeof item['accessCount'] === 'number' ? Math.max(0, Math.floor(item['accessCount'])) : 0
...
function parseLocalDumpTasks(characterId: string, userId: string, raw: unknown): MemoryWriteTask[] {
  if (!Array.isArray(raw)) return []
  const now = Date.now()
  return raw
    .filter(isRecord)
    ...
    .map((item): MemoryWriteTask | null => {
      const id =
        typeof item['id'] === 'string' && item['id'].trim().length > 0 ? item['id'].trim() : null
      const description =
        typeof item['description'] === 'string' ? clip(item['description'], 200) : null
...
function mapCloudEntry(
  row: typeof wikiEntries.$inferSelect,
  firebaseUid: string,
): MemoryWriteEntry {
  return {
    id: row.id,
...
function buildWriteDiffHeuristic(
  ...
  const tasks: MemoryWriteTask[] = pieces
    ...
    .map((piece, index) => ({
      id: useStableIds
        ? stableId('task', characterId, clip(piece, 64).toLowerCase())
...
function buildWriteDiffFromLLMResult(
  ...
  const now = Date.now()
  const defaultConfidence: MemoryWriteEntry['confidence'] =
    sourceType === 'user_document' ? 'certain' : 'inferred'
  const defaultSourceType: MemoryWriteEntry['sourceType'] =
    sourceType === 'user_document' ? 'user_stated' : 'agent_inferred'
  const entries: MemoryWriteEntry[] = []
  const events: MemoryWriteEvent[] = []
  let entriesAdded = 0
  let entriesUpdated = 0
...
async function buildWriteDiff(
  ...
  return buildWriteDiffHeuristic(
    characterId,
    firebaseUid,
    sourceText,
    sourceType,
...
function taskAlreadyCoveredByEntries(task: MemoryWriteTask, entries: MemoryWriteEntry[]): boolean {
  ...
  return entries.some((entry) =>
    `${entry.title} ${entry.body}`.toLowerCase().includes(title.slice(0, 20)),
  )
}
...
async function buildHealDiff(
  ...
  const contradictionPairs = await detectContradictions(
    contradictionCandidates,
    deps.generateContent,
  )
  const contradictedIds = new Set(contradictionPairs.flatMap((p) => [p.entryAId, p.entryBId]))
  const events: MemoryWriteEvent[] = []
  ...
  for (const pair of contradictionPairs) {
    ...
    events.push({
      ...
      eventType: 'observation',
      summary: clip(
        `Contradiction detected: "${older.title}" conflicts with another entry. ${pair.reason}`,
        200,
      ),
...
export const memoryReadHandler = async (
  ...
  const [entriesRows, taskRows, eventRows] = await Promise.all([
    db.select().from(wikiEntries).where(entryWhere).orderBy(desc(wikiEntries.updatedAt)).limit(10),
    db
      .select()
      .from(agentTasks)
      .where(
    ...
    db
      ...
      .from(memoryEvents)
      .where(
        and(eq(memoryEvents.characterId, characterId), eq(memoryEvents.userId, identity.userId)),
      )
...
export const memoryWriteHandler = async (
  ...
  const ownsCharacter = await hasOwnedCloudCharacter(deps, characterId, identity.userId)
  const seedEntries = ownsCharacter
    ? await loadWriteSeed(deps, characterId, identity.userId, identity.firebaseUid)
    : []

  let spendAllocations: CreditSpendAllocation[] | null = null
  try {
    spendAllocations = await deps.creditService.spendCredits(identity.userId, MEMORY_ACTION_COST, 'memory_action')
    ...
    if (ownsCharacter) {
      ...
      for (const entry of diff.entries) {
        entry.syncedToCloud = 1
        entry.cloudId = entry.id
      }
      for (const task of diff.tasks) {
        task.syncedToCloud = 1
        task.cloudId = task.id
      }
      for (const event of diff.events) {
        event.syncedToCloud = 1
        event.cloudId = event.id
      }
    }
...
export const syncCharacterMemoryHandler = async (
  ...
  const [entryCount, taskCount, eventCount] = await Promise.all([
    db
      ...
      .from(wikiEntries)
      .where(
        and(eq(wikiEntries.characterId, characterId), eq(wikiEntries.userId, identity.userId)),
      ),
    db
      .select({ count: sql<number>`count(*)::int` })
      .from(agentTasks)
