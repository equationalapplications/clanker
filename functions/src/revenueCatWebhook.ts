...
export function parseRevenueCatEvent(body: unknown): RevenueCatEvent {
  ...
  const priceInPurchasedCurrency = optionalNumber(
  ...
  return {
    event: {
      ...
      ...(priceInPurchasedCurrency !== undefined
...
export const revenueCatWebhookHandler = async (
  ...
  const bearerMatch =
    typeof normalizedHeader === 'string' ? normalizedHeader.match(/^Bearer\s+/i) : null
  ...
  try {
    ...
    const bodyConstructor =
      bodyForDiagnostics !== null &&
      bodyForDiagnostics !== undefined &&
      'constructor' in Object(bodyForDiagnostics)
        ? (Object(bodyForDiagnostics).constructor?.name ?? 'unknown')
        : 'none'
    const topLevelKeys =
      bodyForDiagnostics &&
      typeof bodyForDiagnostics === 'object' &&
      !Buffer.isBuffer(bodyForDiagnostics)
    ...
    switch (type) {
        ...
        if (REVENUECAT_PRODUCT_TO_TIER[normalizedProductId]) {
          ...
          const expirationDate =
            typeof expiration_at_ms === 'number' && Number.isFinite(expiration_at_ms)
              ...
              : null
          const renewalAt =
            expirationDate && Number.isFinite(expirationDate.getTime()) ? expirationDate : null
          ...
          if (
            ...
            logger.warn(
              'billing_provider_collision: RevenueCat purchase granted while an active Stripe subscription exists',
              {
                app_user_id,
          ...
          if (renewalAt && original_transaction_id && typeof expiration_at_ms === 'number') {
            ...
            await deps.renewSubscriptionCredits(
              cloudUser.id,
              SUBSCRIPTION_RENEWAL_CREDIT_AMOUNT,
              renewalAt,
              referenceId,
          ...
          if (!original_transaction_id) {
            logger.warn(
              'RevenueCat: credit-pack event missing original_transaction_id, rejecting so RevenueCat retries',
              {
                app_user_id,
                product_id,
                type,
      ...
      case 'PRODUCT_CHANGE': {
        if (REVENUECAT_PRODUCT_TO_TIER[normalizedProductId]) {
          ...
          if (
            ...
            logger.warn(
              'billing_provider_collision: RevenueCat product change granted while an active Stripe subscription exists',
              {
                app_user_id,
                existingTier: existingSubscription.planTier,
                newTier: tier,
      ...
      case 'NON_RENEWING_PURCHASE': {
        if (isRevenueCatCreditPackProduct(product_id)) {
          if (!original_transaction_id) {
            logger.warn(
              'RevenueCat: non-renewing credit-pack event missing original_transaction_id, rejecting so RevenueCat retries',
              {
                app_user_id,
                product_id,
                type,
      ...
      case 'CANCELLATION': {
        ...
        if (tier) {
          if (cancel_reason === 'CUSTOMER_SUPPORT') {
            ...
            const clawbackKey =
              typeof expiration_at_ms === 'number'
                ? String(expiration_at_ms)
                : transaction_id
            ...
            if (original_transaction_id && clawbackKey) {
              ...
              logger.warn(
                'RevenueCat: subscription refund missing both expiration_at_ms and transaction_id, cannot claw back',
                { app_user_id, product_id, tier },
              )
            }
            await emitRevenueCatRefund(deps, rcEvent, tier)
            logger.info('RevenueCat: subscription refund — downgraded and clawed back', {
              app_user_id,
              product_id,
              tier,
            })
            ...
            logger.info(
              'RevenueCat: subscription cancellation recorded (auto-renew off, entitlement still active)',
              {
                app_user_id,
                product_id,
                tier,
          ...
          if (original_transaction_id) {
            ...
            logger.info('RevenueCat: credit-pack refund deducted', {
              app_user_id,
              product_id,
            ...
            logger.warn(
              'RevenueCat: credit-pack cancellation missing original_transaction_id, cannot deduct',
              { app_user_id, product_id },
            )
          }
        } else {
          // Neither a known tier nor a known pack — log only, never downgrade.
          logger.warn('RevenueCat: cancellation for unknown product, no state change', {
            app_user_id,
            product_id,
          })
        }
      ...
      case 'EXPIRATION': {
        if (REVENUECAT_PRODUCT_TO_TIER[normalizedProductId]) {
          ...
          logger.info('RevenueCat: expiration for non-subscription product, no state change', {
            app_user_id,
            product_id,
          })
        }
      ...
      case 'UNCANCELLATION': {
        ...
        if (tier) {
          ...
          logger.info('RevenueCat: uncancellation — auto-renew re-enabled', {
            app_user_id,
            product_id,
            tier,
          })
        } else {
          logger.info('RevenueCat: uncancellation for non-subscription product, no state change', {
            app_user_id,
            product_id,
          })
        }
      ...
      case 'BILLING_ISSUE': {
        ...
        logger.warn('RevenueCat: billing issue (grace period, entitlement still active)', {
          app_user_id,
          product_id,
        })
        break
