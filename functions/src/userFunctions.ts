...
function validateUpdateUserProfilePayload(data: unknown): UpdateUserProfilePayload {
  ...
  const unknownKeys = Object.keys(data).filter(
    (key) => !allowedKeys.includes(key as keyof UpdateUserProfilePayload),
  )
    ...
    if (data.defaultCharacterId === null) {
      ...
      if (trimmed.length === 0) {
        ...
        throw new HttpsError(
          'invalid-argument',
          'defaultCharacterId must be a valid UUID when provided.',
        )
      } else {
      ...
      throw new HttpsError(
        'invalid-argument',
        'defaultCharacterId must be a string, null, or undefined.',
      )
    }
  }
