...
test('parseRevenueCatEvent rejects invalid required fields', () => {
  ...
  assert.throws(
    () =>
      parseRevenueCatEvent({
        event: { type: 'INITIAL_PURCHASE', app_user_id: '', product_id: 'prod_1' },
      }),
  ...
  assert.throws(
    () =>
      parseRevenueCatEvent({
        event: { type: 'INITIAL_PURCHASE', app_user_id: 'uid_123', product_id: '' },
      }),
...
test('revenueCatWebhookHandler normalizes expiration to free tier', async () => {
  ...
  const upsertCalls: Array<
    Pick<
      RevenueCatUpsertParams,
      'userId' | 'planTier' | 'planStatus' | 'subscriptionProvider' | 'cancelAtPeriodEnd'
    >
  > = []
...
test('revenueCatWebhookHandler tags new subscriptions with the revenuecat provider', async () => {
  ...
  await revenueCatWebhookHandler(
    ...
    res as never,
    {
      findUserByFirebaseUid: async () => ({ id: 'cloud-user-1' }),
      getSubscription: async () => null,
      upsertSubscription: async (params: RevenueCatUpsertParams) => {
        upsertCalls.push(params)
      },
      renewSubscriptionCredits: async () => false,
      addCredits: async () => undefined,
...
test('revenueCatWebhookHandler downgrades and claws back on a subscription refund', async () => {
  ...
  await revenueCatWebhookHandler(
    ...
    res as never,
    {
      ...
      renewSubscriptionCredits: async () => false,
      addCredits: async () => undefined,
      adjustCredits: async (_uid, delta, reason, referenceId) => {
        adjustCalls.push({ delta, reason, referenceId })
      },
      sendPurchaseEvent: async () => undefined,
      sendRefundEvent: async () => undefined,
    },
  ...
  await revenueCatWebhookHandler(
    {
      ...
      headers: { authorization: 'Bearer rc-secret' },
      body: {
        event: { type: 'CANCELLATION', app_user_id: 'uid_123', product_id: 'some_unknown_product' },
      },
    } as never,
    res as never,
    {
  ...
  await revenueCatWebhookHandler(
    {
      ...
      headers: { authorization: 'Bearer rc-secret' },
      body: {
        event: { type: 'EXPIRATION', app_user_id: 'uid_123', product_id: 'credit_pack_100' },
      },
    } as never,
    res as never,
    {
